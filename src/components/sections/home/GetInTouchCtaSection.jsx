...
/**
 * GetInTouchCtaSection Component ("Let's Build Together - The right workspace today, a bigger tomorrow")
 *
 * Features:
 * - 100% Global Theme tokens (No custom hex codes)
 * - Zero SVG tags (Pure FontAwesome WebFont icons)
 * - Exact replica of the reference CTA banner design
 * - Transparent PNG building cutout (/images/cta/GET IN TOUCH.png) seamlessly integrated on the left
 * - Slogan: SPACES / PEOPLE / POSSIBILITIES on the far left
 * - Headline + "GET IN TOUCH" circular gold action button
 * - 3 Real Metrics: 500+ Spaces Available | 1,000+ Businesses Served | 98% Client Satisfaction
 * - Luxury hover micro-animations & fully responsive for all screen sizes
 */
export default function GetInTouchCtaSection({ onOpenForm }) {
  ...
  return (
    ...
    >
      <Container className="relative z-10">
        ...
        <div
          className={`relative rounded-2xl sm:rounded-3xl bg-gradient-to-r from-[var(--gold-light)] via-[var(--bg-surface)] to-[var(--gold-light)] border border-[var(--gold-border)]/70 shadow-[0_8px_30px_-6px_rgba(10,35,60,0.06)] overflow-hidden transition-opacity duration-700 ${
            isInView ? "animate-fade-up" : "opacity-0"
          }`}
        >
          {/* Far-Left Vertical Tagline (Spaces / People / Possibilities) */}
          ...
          {/* Left Side: Transparent PNG Architecture Cutout (Exact Match to Screenshot) */}
          ...
          {/* Right Side: Header, Headline, CTA Button & 3 Metrics */}
          <div className="relative z-20 lg:ml-auto w-full lg:w-[57%] xl:w-[55%] p-4 sm:p-8 lg:py-9 lg:pr-8 xl:pr-10 lg:pl-4 flex flex-col justify-between">
            {/* Top Row: Subtitle + Headline + Action Button */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sm:gap-6 pb-5 sm:pb-8 border-b border-[var(--gold-border)]/60">
              ...
              <div className="flex-1 text-left">
                ...
                {/* Exact 2-Line Headline for all screens */}
              ...
              {/* Action Button: Circular Gold Button + "GET IN TOUCH" */}
              <div className="flex items-center sm:border-l sm:border-[var(--gold-border)]/60 sm:pl-6 shrink-0 self-start sm:self-center mt-1 sm:mt-0">
                ...
                >
                  {/* Circular Gold Button with Hover Scale */}
            ...
            {/* Bottom Row: 3 Real Statistics / Metrics */}
                ...
                return (
                  <div
                    ...
                    className={`flex flex-col text-left ${
                      !isLast ? "border-r border-[var(--gold-border)]/60 pr-1.5 sm:pr-4" : ""
                    }`}
                  >
