...
import { sectorsData as defaultSectors } from "@/data/sectorsData";
import { useInView } from "@/hooks/useInView";

/**
 * PopularSectorsSection Component
 * Displays popular office sector locations across Noida.
 *
 * Backend Ready:
 * Accepts `sectors` as a prop. When backend API is connected, data fetched
 * from the database/API can be passed directly to this component.
 */
  ...
  return (
    ...
    >
      <Container>
        {/* Section Header */}
        <div
          className={`flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6 transition-opacity duration-500 ${isInView ? "animate-fade-up" : "opacity-0"
            }`}
        >
          {/* Left Title & Eyebrow */}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5 lg:gap-5.5 mt-8 sm:mt-10 lg:mt-12">
          {sectors.map((sector, idx) => (
            ...
            >
              {/* Sector Card Image (Widescreen landscape aspect ratio) */}
              <div className="relative w-full h-[175px] sm:h-[185px] lg:h-[195px] xl:h-[200px] overflow-hidden bg-[var(--primary)]">
                ...
                {/* Deep Gradient Overlay for high-contrast crisp text legibility */}
                ...
                <div className="absolute inset-x-0 bottom-0 p-3 sm:p-3.5 lg:p-4 flex items-end justify-between gap-2.5">
                  {/* Text Container with truncate to guarantee single-line fit */}
                  ...
                  {/* Golden Circular Arrow Action Button (Transparent background matching Reference UI) */}
        ...
        {/* Bottom CTA Banner ("We'll help you find the right space.") */}
        <div
          className={`mt-8 sm:mt-12 bg-[var(--gold-light)] rounded-2xl sm:rounded-3xl p-5 sm:p-7 lg:p-8 border border-[var(--gold-border)]/70 relative overflow-hidden shadow-sm flex flex-col md:flex-row md:items-center md:justify-between gap-5 transition-opacity duration-500 ${isInView ? "animate-fade-up delay-300" : "opacity-0"
            }`}
        >
