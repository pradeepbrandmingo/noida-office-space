...
/**
 * SpaceCategoriesSection Component ("What are you looking for?")
 * Displays 6 interactive office space category cards:
 * 1. Startup Office
 * 2. 10 – 25 Seats
 * 3. 25 – 50 Seats
 * 4. 50 – 100 Seats
 ...
 *
 * Click handler ready for Popup Modal form integration.
 */
export default function SpaceCategoriesSection({ onSelectCategory }) {
  const [sectionRef, isInView] = useInView({ threshold: 0.1, triggerOnce: true });
  const [selectedId, setSelectedId] = useState(null); // No card active by default; active on click or hover
  ...
  return (
    ...
    >
      ...
      <Container className="relative z-10">
        {/* Section Header (Center Aligned matching reference screenshot) */}
        <div
        ...
        {/* 6 Category Cards Grid (Compact top/bottom height, active on click/hover only) */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3.5 sm:gap-4 lg:gap-4.5">
          {categories.map((category, idx) => {
            ...
            return (
              ...
              >
                {/* Center Icon with Soft Circular Glow */}
                <div
